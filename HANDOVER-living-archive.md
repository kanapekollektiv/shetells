...
Single self-contained file, ~1650 lines, no build step. HTML + CSS + JS inline.
...
| Cards | `CARDS` | `[cats[], x, y, w, h, title, content]` |
...
- `{ img, alt }` + optional `link`, `credit`, `full`, `sources`, `note`, `mWidth`
- `{ vid }` + optional `poster` — lazily played by `updateVideos()`, **paused below 0.45 zoom**, which is why `poster` matters
- `{ gallery: [{src, alt}] }` — carousel. Frames ending `.mp4` render as video. Optional `full`, `entries`, `sources`, `note`
...
- **Mobile layout** under 768px: the map is replaced by a plain list of clusters,
  ...
  as they have cards.
- **Carousel** (`gallery`) with arrows, counter, video frames, mobile scroll-snap
  strip. Single-frame galleries hide their chrome.
